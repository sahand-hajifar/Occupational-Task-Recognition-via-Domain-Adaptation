...
kerSigma = param.sigma; % kernel parameter, see the next cell ("kernels")
bSstca = true; % 0 for TCA if no label information is considered for all 
...
m = param.m; % the dimension of the subspace

% SSTCA params
lambda = 1; % weight of the geometry term
knn = 5; % #neighbor when computing the Laplacian matrix
geoSigma = .01;
gamma = 0.1; % the weight of the supervised term, only useful in SMIDA, see 
	% the ref
